...
def fetch_ca_comparison(
    ...
    """Fetch CA mensuel + calcul prévisions basées sur le taux YTD.

    Logique :
    1. Appel mensuel (12 mois N et N-1) pour le graphique
    2. Calcul YTD_A / YTD_B à partir des données mensuelles → taux YTD
    3. Prévision fin de mois en cours : CA_N-1 × (1 + taux_YTD)
    4. Prévision mois futurs : CA_N-1 × (1 + taux_YTD)

    Le taux YTD (cumul depuis le 1er janvier) est plus stable qu'un glissant
    60j et cohérent avec l'indicateur "Évolution" affiché à l'utilisateur.
    """
    ...
    # ── 2. YTD (à date) — prorata linéaire sur le mois en cours ──
    ytd_a = 0.0
    ytd_b = 0.0
    for m in range(1, 13):
    ...
    growth_rate = (ytd_b / ytd_a - 1) if ytd_a > 0 else 0.0

    _log.info(
        "Taux YTD : CA %d YTD=%.0f, CA %d YTD=%.0f → %+.1f%%",
        year_b, ytd_b, year_a, ytd_a, growth_rate * 100,
    )
    ...
    # ── 4. Construire le tableau mensuel + CA cible ──
    ...
    ytd_pct = round(growth_rate * 100, 1)
...
def fetch_ca_comparison_with_tag(
    ...
    Taux basé sur le cumul YTD N vs N-1 (pas le glissant 60j).
    """
    from common.easybeer.indicators import get_ca_mensuel
    ...
    growth_rate = (ytd_b / ytd_a - 1) if ytd_a > 0 else 0.0
    _log.info("Tag '%s' taux YTD: CA %d YTD=%.0f, CA %d YTD=%.0f → %+.1f%%",
              tag, year_b, ytd_b, year_a, ytd_a, growth_rate * 100)
